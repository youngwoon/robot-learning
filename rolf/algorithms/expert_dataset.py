...
class ExpertDataset(Dataset):
    ...
    def __init__(
        ...
        for file_path in demo_files:
            ...
            for demo in demos:
                if is_ob_next and len(demo["obs"]) != len(demo["actions"]) + 1:
                    Logger.error(
                        f"Mismatch in # of observations ({len(demo['obs'])}) and actions ({len(demo['actions'])}): {str(file_path)}"
                    )
                    continue
