    ...
    def __init__(self, cfg, ob_space, ac_space):
        ...
        if cfg.is_train:
            ...
            if cfg.val_split != 0:
                ...
                split = int(np.floor((1 - cfg.val_split) * dataset_size))
                train_indices, val_indices = indices[split:], indices[:split]
                train_sampler = SubsetRandomSampler(train_indices)
                val_sampler = SubsetRandomSampler(val_indices)
                self._train_loader = torch.utils.data.DataLoader(
                    self._dataset, batch_size=cfg.batch_size, sampler=train_sampler
                )
                self._val_loader = torch.utils.data.DataLoader(
                    self._dataset, batch_size=cfg.batch_size, sampler=val_sampler
                )
            else:
                self._train_loader = torch.utils.data.DataLoader(
                    self._dataset, batch_size=cfg.batch_size, shuffle=True
                )
