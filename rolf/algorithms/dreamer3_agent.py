...
class Dreamer3Agent(BaseAgent):
    def __init__(self, cfg, ob_space, ac_space):
        ...
        state_dim = cfg.wm.deter_dim + cfg.wm.stoch_dim
        if cfg.wm.stoch_discrete:
            state_dim = cfg.wm.deter_dim + cfg.wm.stoch_dim * cfg.wm.stoch_discrete

        # Build up networks.
        self.wm = DreamerModel(cfg, ob_space, ac_dim, self._dtype)
        ...
        # torch.compile -- about 10% speed up.
        self.wm = torch.compile(self.wm, disable=not cfg.compile)
    ...
    def _log_creation(self):
        ...
        Logger.info(f"The world model has {count_parameters(self.wm)} parameters")
    ...
    def act(self, ob, state, is_train=True):
        ...
        with torch.autocast(self._cfg.device, enabled=self._use_amp):
            state = state or self.initial_state(len(list(ob.values())[0]))
    ...
    @torch.no_grad()
    def mask_state(self, state, done):
        if state is None:
            return None
        if done.any():
            latent, action = state
            done = to_tensor(done, self._device, self._dtype)
            latent1 = rmap(lambda x: torch.einsum("b...,b->b...", x, 1.0 - done), latent)
            latent2 = rmap(
                lambda x: torch.einsum("b...,b->b...", x, done), self.wm.initial(len(done))
            )
            latent = {k: latent1[k] + latent2[k] for k in latent}
            action = torch.einsum("b...,b->b...", action, 1.0 - done)
    ...
    def load_state_dict(self, ckpt):
        ...
        self.wm_optim.load_state_dict(ckpt["wm_optim"])
        self.actor_optim.load_state_dict(ckpt["actor_optim"])
        self.critic_optim.load_state_dict(ckpt["critic_optim"])
        optimizer_cuda(self.wm_optim, self._device)  # TODO: remove it
    ...
    def update(self):
        ...
        for _ in range(self._cfg.train_iter):
            ...
            sw.stop("train")
        Logger.info(f"Data: {sw.average('data'):.3f}  Train: {sw.average('train'):.3f}")

        info = train_info.get_dict()
        # Logger.info(
        #     f"wm_grad: {info['wm_grad_norm']:.1f} / actor_grad:
        #     {info['actor_grad_norm']:.1f} / critic_grad:
        #     {info['critic_grad_norm']:.1f} / wm_loss: {info['wm_loss']:.1f} / actor_loss: {info['actor_loss']:.1f} / critic_loss: {info['critic_loss']:.1f} / value_target: {info['value_target']:.1f} / value_predicted: {info['value_predicted']:.1f} / prior_ent: {info['prior_entropy']:.1f} / post_ent: {info['posterior_entropy']:.1f} / reward_loss: {info['reward_loss']:.1f} / div: {info['kl_loss']:.1f} / actor_ent: {info['actor_entropy']:.1f}"
        # )
        return info

    def _update_wm(self, o, ac, rew, cont, is_first):
        info = Info()
        cfg = self._cfg

        with RequiresGrad(self.wm):
            with torch.autocast(cfg.device, enabled=self._use_amp):
                state = self.wm.initial(ac.shape[0])
                embed = self.wm.encoder(o)
                ac[:, 0, ...] *= 0  # The first action is not used.
                # post, prior = self.wm.observe(embed, ac, is_first)
                post, prior = self.wm(embed, ac, is_first, state)
                feat = self.wm.get_feat(post)

                ob_pred = self.wm.decoder(feat)
                recon_losses = {k: -ob_pred[k].log_prob(v).mean() for k, v in o.items()}
                recon_loss = sum(recon_losses.values())

                reward_pred = self.wm.reward(feat)
                reward_loss = -reward_pred.log_prob(rew.unsqueeze(-1)).mean()

                cont_pred = self.wm.cont(feat)
                cont_loss = -cont_pred.log_prob(cont.unsqueeze(-1)).mean()

                prior_dist = self.wm.get_dist(prior)
                post_dist = self.wm.get_dist(post)

                prior_dist_sg = self.wm.get_dist_sg(prior)
                post_dist_sg = self.wm.get_dist_sg(post)
                ...
                wm_loss = div_clipped + recon_loss + reward_loss + cont_loss
            wm_grad_norm = self.wm_optim.step(wm_loss)
        ...
        info["wm_loss"] = wm_loss.item()
        info["wm_grad_norm"] = wm_grad_norm.item()

        return post, prior, embed, ob_pred, info

    def _update_actor(self, post, cont):
        ...
        with RequiresGrad(self.actor):
            with torch.autocast(cfg.device, enabled=self._use_amp):
                ...
                with torch.no_grad():
                    imagine_reward = (
                        self.wm.reward.forward(imagine_feat).mode.squeeze(-1).float()
                    )
        ...
        return imagine_feat, imagine_return, discount, info

    def _update_critic(self, imagine_feat, imagine_return, discount):
        info = Info()
        cfg = self._cfg
    ...
    def _update_network(self, batch, log_image=False):
        ...
        ac = batch["ac"]
        rew = batch["rew"]
        cont = 1 - batch["terminated"].type(self._dtype)
        done = batch["done"].type(self._dtype)
        is_first = torch.cat([torch.ones_like(done[:, :1]), done[:, :-1]], -1)
        o = self.preprocess(o)

        post, prior, embed, ob_pred, info_ = self._update_wm(o, ac, rew, cont, is_first)
        info.add(info_)
        imagine_feat, imagine_return, discount, info_ = self._update_actor(post, cont)
        info.add(info_)
        info_ = self._update_critic(imagine_feat, imagine_return, discount)
        info.add(info_)
        ...
        if log_image and False:
            with torch.no_grad(), torch.autocast(cfg.device, enabled=self._use_amp):
                ...
                init, _ = self.wm.observe(embed[:4, :5], ac[:4, :5], is_first[:4, :5])
                init = {k: v[:, -1] for k, v in init.items()}
                prior = self.wm.imagine(ac[:4, 5:], init)
                openloop = self.wm.decoder(self.wm.get_feat(prior)).mode
                for k, v in o.items():
                    if len(v.shape) != 5:
                        continue
                    truth = o[k][:4] + 0.5
        ...
        if getattr(cfg, "maze_visualize", False):
            with torch.no_grad():
                init, _ = self.wm.observe(embed[:1, :1], ac[:1, :1])
                init = {k: v[:, -1] for k, v in init.items()}
                prior = self.wm.imagine(ac[:1, 1:], init)
                openloop = self.wm.decoder(self.wm.get_feat(prior)).mode
                for k, v in o.items():
    ...
    def _imagine_ahead(self, post, cont):
        ...
        start["action"] = self.actor.act(self.wm.get_feat(start).detach())

        def rollout(prev_state, _):
            state = self.wm.imagine_step(prev_state, prev_state["action"])
            state["action"] = self.actor.act(self.wm.get_feat(state).detach())
            return state
    ...
    def _policy(self, ob, state, is_train):
        ...
        embed = self.wm.encoder(self.preprocess(ob))
        try:
            latent = self.wm.obs_step(prev_latent, prev_action, embed)
    ...
    def initial_state(self, batch_size):
        latent = self.wm.initial(batch_size)
        action = torch.zeros(
